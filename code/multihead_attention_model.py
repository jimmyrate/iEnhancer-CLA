...
def residual_layer_2016(indata, out_channel, i_bn=False, k1=3, k2=3, strides=None, dropout=0, actfun='relu'):
    conv_out1 = conv1d_bn(indata, out_channel, k1, dropout=dropout, padding='same', use_bias=False, BN=i_bn,
                          active_function=actfun, pre_activation=True)
    conv_out2 = conv1d_bn(conv_out1, out_channel, k2, dropout=dropout, padding='same', use_bias=False, BN=i_bn,
                          active_function=actfun, pre_activation=True)
...
class multihead_attention:
    ...
    def build_model_multihead_attention_multiscaleCNN4_covermore(self,
        ...
        ###print(posembed)
        ###print(posmod)
        input = Input(shape=(self.max_len,), dtype='float16')
        ...
        with tf.name_scope('first_cnn'):
            ...
            first_cnn2 = Convolution1D(int(nb_filters), filters_length1,  # kernel_regularizer=regularizers.l2(0.0001),
                                       border_mode='same', activation=activationlastfun, use_bias=False)(
                embedding_output)  # layer5
            ...
            second_cnn2 = Convolution1D(int(nb_filters), filters_length2,  # kernel_regularizer=regularizers.l2(0.0001),
                                        border_mode='same', activation=activationlastfun, use_bias=False)(
                embedding_output)
            # third_cnn = Convolution1D(int(nb_filters / 2), filters_length3,
            ...
            #                          border_mode='same', activation=activationlastfun, use_bias=False)(embedding_output)
        ...
        mask_input=[]
        mask_input.append(cnn_output1)
        mask_input.append(input_mask)
        mask_layer1 = Lambda(mask_func)(mask_input)
        del mask_input
        mask_input = []
        mask_input.append(cnn_output2)
        mask_input.append(input_mask)
        mask_layer2 = Lambda(mask_func)(mask_input)
        del mask_input
        # mask_input = []
        # mask_input.append(cnn_output3)
        # mask_input.append(input_mask)
        # mask_layer3 = Lambda(mask_func)(mask_input)
        # del mask_input
        with tf.name_scope('bilstm_layer'):
            first_lstm = layers.Bidirectional(
                CuDNNLSTM(dim_lstm, kernel_initializer=kernel_initializer, recurrent_initializer=recurrent_initializer,
                          return_sequences=True))(mask_layer1)
            lstm1_output = layers.BatchNormalization()(first_lstm)
            drop1_lstm = layers.Dropout(drop_lstm)(lstm1_output)
            second_lstm = layers.Bidirectional(
                CuDNNLSTM(dim_lstm, kernel_initializer=kernel_initializer, recurrent_initializer=recurrent_initializer,
                          return_sequences=True))(mask_layer2)
            lstm2_output = layers.BatchNormalization()(second_lstm)
            drop2_lstm = layers.Dropout(drop_lstm)(lstm2_output)
            # third_lstm = layers.Bidirectional(CuDNNLSTM(dim_lstm3,kernel_initializer="orthogonal", recurrent_initializer="orthogonal",
            #                                     return_sequences=True))(mask_layer3)
            # lstm3_output = layers.BatchNormalization()(third_lstm)
            # drop3_lstm = layers.Dropout(drop_lstm)(lstm3_output)

        mask_input1 = []
        mask_input1.append(drop1_lstm)
        mask_input1.append(input_mask)
        cnn_mask_output1 = Lambda(mask_func)(mask_input1)
        del mask_input1
        mask_input2 = []
        mask_input2.append(drop2_lstm)
        mask_input2.append(input_mask)
        cnn_mask_output2 = Lambda(mask_func)(mask_input2)
        del mask_input2
        # mask_input3 = []
        # mask_input3.append(drop3_lstm)
        # mask_input3.append(input_mask)
        # cnn_mask_output3 = Lambda(mask_func)(mask_input3)
        # del mask_input3

        with tf.name_scope('multihead_attention'):
            att1, att1_A = Attention_mask(hidden=drop1_lstm.get_shape()[-1].value, da=dim_attention, r=headnum,
                                          ...
                                          attention_regularizer_weight=Att_regularizer_weight, normalize=normalizeatt,
                                          attmod=attmod, sharp_beta=sharp_beta, name="att1")(
                concatenate([cnn_mask_output1, input_mask]))  # -5 layer

            att2, att2_A = Attention_mask(hidden=drop1_lstm.get_shape()[-1].value, da=dim_attention, r=headnum,
                                          init='glorot_uniform', activation='tanh',
                                          W1_regularizer=regularizerfunction_W1,
                                          W2_regularizer=regularizerfunction_W2,
                                          W1_constraint=None, W2_constraint=None, return_attention=True,
                                          attention_regularizer_weight=Att_regularizer_weight, normalize=normalizeatt,
                                          attmod=attmod, sharp_beta=sharp_beta, name="att2")(
                concatenate([cnn_mask_output2, input_mask]))  # -4 layer
            """
            att3, att3_A = Attention_mask(hidden=drop1_lstm.get_shape()[-1].value, da=dim_attention, r=headnum,
                                          init='glorot_uniform', activation='tanh',
                                          W1_regularizer=regularizerfunction_W1,
                                          W2_regularizer=regularizerfunction_W2,
                                          W1_constraint=None, W2_constraint=None, return_attention=True,
                                          attention_regularizer_weight=Att_regularizer_weight, normalize=normalizeatt,
                                          attmod=attmod, sharp_beta=sharp_beta, name="att3")(
                concatenate([cnn_mask_output3, input_mask]))  # -3 layer
            """
            if BatchNorm:
            ...
            output = Dropout(drop_flat)(Flatten()(concatenate([att1, att2])))
        fc = output
        ...
        optim = optimizers.Adam(lr=lr,
                                decay=5e-5)  # The paper uses a decay rate alpha = alpha/sqrt(t) updted each epoch (t) for the logistic regression demonstration.
    ...
    def get_attention_multiscale_batch(self, X, X_mask):
        ...
        #return attmodel1.predict([X, X_mask.reshape(-1, X_mask.shape[1], 1)], batch_size=100), attmodel2.predict(
        #    [X, X_mask.reshape(-1, X_mask.shape[1], 1)], batch_size=100), attmodel3.predict(
        #    [X, X_mask.reshape(-1, X_mask.shape[1], 1)], batch_size=100)
        return attmodel1.predict([X, X_mask.reshape(-1, X_mask.shape[1], 1)], batch_size=100), attmodel2.predict(
            [X, X_mask.reshape(-1, X_mask.shape[1], 1)], batch_size=100)

    def train(self, x_train, y_train, mask_label, batch_size, epochs=100, x_valid=None, y_valid=None, mask_valid=None,
              loadFinal=False):
        ...
        hist = self.model.fit([x_train, mask_train.reshape(-1, mask_train.shape[1], 1)], y_train,
                                  batch_size=batch_size, nb_epoch=epochs, verbose=1,
                                  validation_data=([x_valid, mask_valid.reshape(-1, mask_valid.shape[1], 1)], y_valid),
                                  callbacks=[model_checkpoint, early_stopping], shuffle=True)
    ...
    def train_optimizor(self, x_train, y_train, mask_train, x_test, y_test, mask_test, batch_size, epochs=100):
        ...
        hist = self.model.fit([x_train, mask_train.reshape(-1, x_train.shape[1], 1)], y_train, batch_size=batch_size,
                              nb_epoch=epochs, verbose=1,
                              validation_data=([x_test, mask_test.reshape(-1, x_test.shape[1], 1)], y_test),
                              shuffle=True)
    ...
    def evaluate(self, x_test, y_test, mask_label):
        ...
        np.savetxt(OUTPATH + 'testevaluation_roc_average_presicion_fold_{}.txt'.format(self.kfold_index),
                   np.array(roc_list + pr_list + mcc_list), delimiter=',')
    ...
    def train_regression(self, x_train, y_train, mask_label, batch_size, epochs=100, x_valid=None, y_valid=None,
                         mask_valid=None, loadFinal=False):
        if not self.is_built:
            ##print('Run build_model() before calling train opertaion.')
            return
        ...
        ###print(self.model.evaluate(x_train, y_train, batch_size=batch_size))
        ###print(self.model.evaluate(x_valid, y_valid, batch_size=batch_size))
        hist = self.model.fit([x_train, mask_train.reshape(-1, mask_train.shape[1], 1)], y_train, batch_size=batch_size,
                              nb_epoch=epochs, verbose=1,
                              validation_data=([x_valid, mask_valid.reshape(-1, mask_valid.shape[1], 1)], y_valid),
                              callbacks=[model_checkpoint, early_stopping], shuffle=True)
    ...
    def get_PCM_multiscale_weighted(self, X, mask_label, nb_filters, filters_length1, filters_length2, filters_length3):
        ...
        feature_model1 = Model(self.model.inputs,
                               self.model.get_layer('CNN1').output)  # layer 2 is cnn for 1CNN model no dropout!
        ...
        for m in range(nb_filters):
            ...
            for s in range(len(X)):
                ...
                CNNoutputs1 = feature_model1.predict(
                    [X[s:s + 1], mask_label[s:s + 1].reshape(-1, mask_label[s:s + 1].shape[1], 1)], batch_size=50)
                CNNoutputs2 = feature_model2.predict(
                    [X[s:s + 1], mask_label[s:s + 1].reshape(-1, mask_label[s:s + 1].shape[1], 1)], batch_size=50)
                sub_index1 = CNNoutputs1[0, :, m].argmax() - Add1up
                sub_index2 = CNNoutputs2[0, :, m].argmax() - Add2up
                if m < int(nb_filters / 2):
                    CNNoutputs3 = feature_model3.predict(
                        [X[s:s + 1], mask_label[s:s + 1].reshape(-1, mask_label[s:s + 1].shape[1], 1)], batch_size=50)
                ...
                if CNNoutputs1[0, :, m].max() > 0:
                    if sub_index1 >= 0 and sub_index1 + filters_length1 < onehotX.shape[1]:
                        PCM1 = PCM1 + onehotX[s, sub_index1:(sub_index1 + filters_length1), :] * CNNoutputs1[0, :,
                                                                                                 m].max()
                    elif sub_index1 < 0:
                        PCM1 = PCM1 + np.pad(onehotX[s, 0:sub_index1 + filters_length1, :], ([-sub_index1, 0], [0, 0]),
                                             'constant', constant_values=0) * CNNoutputs1[0, :,
                                                                              m].max()  # add zeros before

                    else:
                        PCM1 = PCM1 + np.pad(onehotX[s, sub_index1:, :],
                                             ([0, filters_length1 - onehotX.shape[1] + sub_index1], [0, 0]), 'constant',
                                             constant_values=0) * CNNoutputs1[0, :, m].max()  # add zeros after
                if CNNoutputs2[0, :, m].max() > 0:
                    if sub_index2 >= 0 and sub_index2 + filters_length2 < onehotX.shape[1]:
                        PCM2 = PCM2 + onehotX[s, sub_index2:(sub_index2 + filters_length2), :] * CNNoutputs2[0, :,
                                                                                                 m].max()
                    elif sub_index2 < 0:
                        PCM2 = PCM2 + np.pad(onehotX[s, 0:sub_index2 + filters_length2, :], ([-sub_index2, 0], [0, 0]),
                                             'constant', constant_values=0) * CNNoutputs2[0, :,
                                                                              m].max()  # add zeros before
                    else:
                        PCM2 = PCM2 + np.pad(onehotX[s, sub_index2:, :],
                                             ([0, filters_length2 - onehotX.shape[1] + sub_index2], [0, 0]), 'constant',
                                             constant_values=0) * CNNoutputs2[0, :, m].max()  # add zeros after
                if m < int(nb_filters / 2):
                    if CNNoutputs3[0, :, m].max() > 0:
                        if sub_index3 >= 0 and sub_index3 + filters_length3 < onehotX.shape[1]:
                            PCM3 = PCM3 + onehotX[s, sub_index3:(sub_index3 + filters_length3), :] * CNNoutputs3[0, :,
                                                                                                     m].max()
                        elif sub_index3 < 0:
                            PCM3 = PCM3 + np.pad(onehotX[s, 0:sub_index3 + filters_length3, :],
                                                 ([-sub_index3, 0], [0, 0]), 'constant',
                                                 constant_values=0) * CNNoutputs3[0, :, m].max()  # add zeros before
                        else:
                            PCM3 = PCM3 + np.pad(onehotX[s, sub_index3:, :],
                                                 ([0, filters_length3 - onehotX.shape[1] + sub_index3], [0, 0]),
                                                 'constant', constant_values=0) * CNNoutputs3[0, :,
                                                                                  m].max()  # add zeros after
            ...
            for s in range(len(X)):
                ...
                if CNNoutputs1[0, :, m].max() > 0:
                    if sub_index1 >= 0 and sub_index1 + filters_length1 < onehotX.shape[1]:
                        ...
                        PCM1 = PCM1 + np.pad(onehotX[s, 0:sub_index1 + filters_length1, :], ([-sub_index1, 0], [0, 0]),
                                             'constant', constant_values=0)  # add zeros before

                    else:
                        PCM1 = PCM1 + np.pad(onehotX[s, sub_index1:, :],
                                             ([0, filters_length1 - onehotX.shape[1] + sub_index1], [0, 0]), 'constant',
                                             constant_values=0)  # add zeros after
                if CNNoutputs2[0, :, m].max() > 0:
                    if sub_index2 >= 0 and sub_index2 + filters_length2 < onehotX.shape[1]:
                ...
                if m < int(nb_filters / 2):
                    if CNNoutputs3[0, :, m].max() > 0:
                        if sub_index3 >= 0 and sub_index3 + filters_length3 < onehotX.shape[1]:
                            ...
                            PCM3 = PCM3 + np.pad(onehotX[s, 0:sub_index3 + filters_length3, :],
                                                 ([-sub_index3, 0], [0, 0]), 'constant',
                                                 constant_values=0)  # add zeros before
                        else:
                            PCM3 = PCM3 + np.pad(onehotX[s, sub_index3:, :],
                                                 ([0, filters_length3 - onehotX.shape[1] + sub_index3], [0, 0]),
                                                 'constant', constant_values=0)  # add zeros after
