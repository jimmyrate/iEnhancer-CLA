...
def get_id_label_seq_Dict(gene_data):
    ...
    for gene in gene_data:
         ...
         id_label_seq_Dict[gene_id][label]= (gene.seqline)
    
    return id_label_seq_Dict


def get_label_id_Dict(id_label_seq_Dict): #读取数据
...
def typeicalSampling(ids, k):   #将数据集分为训练集、测试集、验证集
...
def group_sample(label_id_Dict,datasetfolder,foldnum=5): #将3个数据集的序列标题，分别存储
    Train = OrderedDict()
    Test = OrderedDict()
    Val = OrderedDict()
...
def preprocess_data(left, right,dataset,padmod='center',pooling_size=3):
    gene_data = Gene_data.load_sequence(dataset, left, right) #返回的就是序列
    id_label_seq_Dict = get_id_label_seq_Dict(gene_data)
    #label_id = get_label_id(label) #获得标签
    ...
    if os.path.exists(datasetfolder+'/Train'+str(0)+'.txt'):
        ...
        #[Train, Test,Val] = group_sample(label_id,datasetfolder)
    ...
    Test_mask_label={}
    Val_mask_label={}
    maxpoolingmax = int(right/pooling_size)
    ...
    for i in range(5):
        ...
        #train
        #padd center
        print(id_label_seq_Dict[id].values()[0][0] for id in Train[i])

        x = [[encoding_keys.index(c) for c in list(id_label_seq_Dict[id].values())[0]] for id in Train[i]]
        """
        x_right_data = []
        for id in Train[i]:
            #seq_left = list(id_label_seq_Dict[id].values())[0][0]
            #x_left_data.append(seq_left)
            seq_right = list(id_label_seq_Dict[id].values())[0]
            x_right_data.append(seq_right)
        #x_left_data_array = array(x_left_data)
        x_right_data_array = array(x_right_data)

        for k in range(1, 6):
            Z = getSpectrumProfileMatrix(x_right_data_array, piRNAletter, k)
            X = noramlization(Z)
            if k == 1:
                x_right = X
            else:
                x_right = np.hstack((x_right, X))
        ...
        if padmod =='center':
           ...
           Xtrain[i] = pad_sequences(Xall,maxlen=right,dtype=np.int8, value=encoding_keys.index('UNK'),padding='post')
           #Xtrain[i] = x_right
           #mask_label = np.array([np.concatenate([np.ones(len(gene)),np.zeros(left+right-len(gene))]) for gene in Xall],dtype='float32')
           #Train_mask_label[i]=maxpooling_mask(mask_label,pool_length=pooling_size)
           Train_mask_label[i]=np.array([np.concatenate([np.ones(int(len(gene)/pooling_size)),np.zeros(maxpoolingmax-int(len(gene)/pooling_size))]) for gene in Xall],dtype='float32')#原本序列长度
        ...
        #test##########################################################################################################
        x = [[encoding_keys.index(c) for c in list(id_label_seq_Dict[id].values())[0]] for id in Test[i]]
        """
        x_right_data = []
        for id in Test[i]:
            # seq_left = list(id_label_seq_Dict[id].values())[0][0]
        ...
        if padmod == 'center':
            ...
            mask_label_right = np.array(
                [np.concatenate([np.zeros(right - len(gene)), np.ones(len(gene))]) for gene in X_right],
                dtype='float32')
            mask_label = np.concatenate([mask_label_left, mask_label_right], axis=-1)
            Train_mask_label[i] = maxpooling_mask(mask_label, pool_length=pooling_size)
            X_left = pad_sequences(X_left, maxlen=left,
            ...
            # merge left and right and padding after sequence
            Xall = [np.concatenate([x], axis=-1) for x in x]
            Xtest[i] = pad_sequences(Xall, maxlen=right, dtype=np.int8, value=encoding_keys.index('UNK'),
                                      padding='post')
            # Xtrain[i] = x_right
            # mask_label = np.array([np.concatenate([np.ones(len(gene)),np.zeros(left+right-len(gene))]) for gene in Xall],dtype='float32')
            # Train_mask_label[i]=maxpooling_mask(mask_label,pool_length=pooling_size)
            Test_mask_label[i] = np.array([np.concatenate(
                [np.ones(int(len(gene) / pooling_size)), np.zeros(maxpoolingmax - int(len(gene) / pooling_size))]) for
                                            gene in Xall], dtype='float32')  # 原本序列长度
        ...
        """
        #validation###################################################################################
        x = [[encoding_keys.index(c) for c in list(id_label_seq_Dict[id].values())[0]] for id in Val[i]]
        """
        x_right_data = []
        for id in Val[i]:
            # seq_left = list(id_label_seq_Dict[id].values())[0][0]
        ...
        if padmod == 'center':
            ...
            # merge left and right and padding after sequence
            Xall = [np.concatenate([x], axis=-1) for x in x]
            Xval[i] = pad_sequences(Xall, maxlen=right, dtype=np.int8, value=encoding_keys.index('UNK'),
                                     padding='post')
            # Xtrain[i] = x_right
            # mask_label = np.array([np.concatenate([np.ones(len(gene)),np.zeros(left+right-len(gene))]) for gene in Xall],dtype='float32')
            # Train_mask_label[i]=maxpooling_mask(mask_label,pool_length=pooling_size)
            Val_mask_label[i] = np.array([np.concatenate(
                [np.ones(int(len(gene) / pooling_size)), np.zeros(maxpoolingmax - int(len(gene) / pooling_size))]) for
                gene in Xall], dtype='float32')  # 原本序列长度
...
def run_model(lower_bound, upper_bound, max_len, dataset, foldnum=5 , **kwargs):
    ...
    for i in range(foldnum):
        ...
        model.train(Xtrain[i], Ytrain[i],Train_mask_label[i], kwargs['batch_size'], kwargs['epochs'],Xval[i],Yval[i],Val_mask_label[i],loadFinal=kwargs['loadFinal'])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--drop_input', type=float, default=0.40450826, help='dropout ratio')
    parser.add_argument('--drop_cnn', type=float, default=0.09252784, help='dropout ratio')
    parser.add_argument('--drop_flat', type=float, default=0.18602464, help='dropout ratio')
    ...
    parser.add_argument('--nb_filters', type=int, default=48, help='number of CNN filters')
    parser.add_argument('--dim_lstm', type=int, default=16, help='number of LSTM filters')
    parser.add_argument('--filters_length1', type=int, default=15, help='kernel length for CNN filters1')
    parser.add_argument('--filters_length2', type=int, default=25, help='kernel length for CNN filters2')
    parser.add_argument('--filters_length3', type=int, default=49, help='kernel length for CNN filters3') 
    parser.add_argument('--pooling_size', type=int, default=4, help='pooling_size')
    ...
    parser.add_argument("--nb_classes",type=int,default=3,help = 'nb_classes')
